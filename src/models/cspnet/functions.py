...
from models.cspnet.nms_wrapper import nms
...
def vis_detections(im, class_det, w=None):
    for det in class_det:
        ...
        cv2.rectangle(im, (int(bbox[0]), int(bbox[1])),
                      (int(bbox[2]), int(bbox[3])), (127, 255, 0), 1)
        cv2.putText(im, '{:.3f}'.format(score),
                    (int(bbox[0]), int(bbox[1] - 9)), cv2.FONT_HERSHEY_SIMPLEX,
...
def parse_det_offset(
    pos, height, offset, size, score=0.1, down=4, nms_thresh=0.3
):
    pos = np.squeeze(pos)
    ...
    if len(y_c) > 0:
        for i in range(len(y_c)):
            ...
            s = pos[y_c[i], x_c[i]]
            x1, y1 = max(0, (x_c[i] + o_x + 0.5) * down - w / 2), max(0, (
                y_c[i] + o_y + 0.5) * down - h / 2)
            boxes.append(
                [x1, y1, min(x1 + w, size[1]), min(y1 + h, size[0]), s])
