...
nvcc_compile_args = ['-O3', '--ptxas-options=-v', '-arch=sm_75', '-c',
...
cudamat_ext = Extension('nms.gpu_nms',
                        ...
                        include_dirs=[numpy_include,
                                      'C:\\Programming\\CUDA\\v7.5\\include'])
    ...
    def spawn(self, cmd, search_path=1, verbose=0, dry_run=0):
        ...
        if (sys.platform == 'darwin' and len(cmd) >= 2 and cmd[0] == 'nvcc' and
            ...
            cmd[:1] = [
                'nvcc',
                '--compiler-bindir',
                # os.path.dirname(find_executable("cl.exe", PATH)) or
                os.path.join(os.path.dirname(cmd[0]), 'cl.exe')
            ]
